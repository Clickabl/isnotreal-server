...
const profile = {
  publicId: '42', slug: 'example', name: 'Example', kind: 'person', lists: ['highlight'], reasons: [],
};
const entities = {
  async byPublicId(id) { return id === '42' ? profile : null; },
  async bySlug(slug) { return slug === 'example' ? profile : null; },
  async search() { return [profile]; },
};
const alternatives = { async list() { return []; }, async preferred() { return null; } };
const submissions = {
  async create() { return { id: 'submission-1', submittedAt: '2026-09-15T00:00:00Z', state: 'pending' }; },
};
const publications = {
  async manifest(channel, list) {
    return { schemaVersion: 2, channel, list, version: '1', generatedAt: '2026-09-15T00:00:00Z', expiresAt: '2026-09-16T00:00:00Z' };
  },
...
test('community submission endpoint validates before enqueueing', async () => {
  ...
  const accepted = await route(request('POST', '/api/v1/submissions', {}, {
    submissionType: 'add-evidence', narrative: 'Documented update', sourceUrls: ['https://example.org/source'],
  }));
