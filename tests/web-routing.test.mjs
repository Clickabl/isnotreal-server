...
const entities = {
  async byPublicId(id) {
    return id === '43212' ? entity : null;
  },
  async bySlug(slug) {
    return slug === 'macklemore' ? entity : null;
  },
  async search() {
    return [];
  },
...
const alternatives = {
  ...
  async preferred() {
    return {
      ...
      relationshipType: 'similar-service',
      contextKey: 'general',
      rationale: null,
      destinationUrl: 'https://example.org/',
      destinationChannel: 'domain',
    };
  },
};
...
test('numeric public entity URLs resolve to canonical slugs', async () => {
  ...
  assert.deepEqual(await resolve('/43212'), {
    kind: 'redirect',
    status: 308,
    location: '/macklemore',
  });
...
test('go-to-alt uses a temporary redirect to the current approved alternative', async () => {
  const resolve = createPublicRouteResolver({ entities, alternatives });
  assert.deepEqual(await resolve('/go-to-alt/43212'), {
    kind: 'redirect',
    status: 302,
