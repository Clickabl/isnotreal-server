...
WITH checks AS (
  SELECT
    ...
    EXISTS (
      SELECT 1
      FROM assertion_reasons ar
      WHERE ar.assertion_id = a.id
        AND ar.reason_code = mdr.reason_code
    ) AS assertion_reason_valid,
    (
      crc.default_list IN ('filter', 'highlight')
      AND md.list_kind = crc.default_list
    ) AS list_kind_valid,
    CASE
  ...
  LEFT JOIN LATERAL (
    SELECT
      count(DISTINCT sd.id)::integer AS source_count,
    ...
    WHERE asl.assertion_id = a.id AND asl.stance = 'supports'
  ) ev ON true
)
SELECT
  decision_id,
  ...
  array_remove(
    ARRAY[
      ...
      CASE WHEN NOT list_kind_valid THEN 'reason-list-direction-mismatch' END,
      CASE WHEN NOT assertion_reason_valid THEN 'assertion-reason-mismatch' END,
